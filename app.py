...
# --- 2. STYLE "GALERIE TECHNIQUE" (GRIS & GRENAT, SANS BLOCS) ---
st.markdown("""
<style>
    ...
    /* Élimination totale des rectangles blancs et bordures */
...
# --- 3. FONCTIONS TECHNIQUES (CORRECTION DE LA VALUEERROR) ---
@st.cache_resource
def load_neuro_model():
    ...
    # Correction : On charge le modèle directement au lieu de charger les poids sur une architecture vide
    try:
        model = tf.keras.models.load_model(model_path)
    except Exception:
        # Fallback au cas où le fichier ne contient que les poids
...
def create_clinical_pdf(nom, prenom, age, gender, result, confidence, img, date_str):
    ...
    info = [
        ["Patient Full Name", f"{nom} {prenom}"],
        ["Age / Gender", f"{age} / {gender}"],
        ["Developed By", "Douaa Houbad (M1 EMB)"],
        ["Model Name", "MobileNetV2-NeuroV6"],
    ...
    pdf.ln(10)
    img.save("scan_report.png")
    pdf.image("scan_report.png", x=65, w=80)
    
    pdf.ln(10)
    pdf.set_font("Times", 'B', 14)
    pdf.set_fill_color(240, 240, 240)
    pdf.cell(0, 15, f"DIAGNOSTIC CONCLUSION: {result.upper()}", 1, 1, 'C', True)
...
st.markdown(f"""
    <div class="header-area">
        ...
        <p class="subtitle-text">Engineering & Biomedical Diagnostics</p>
    </div>
""", unsafe_allow_html=True)
...
with c3:
    ...
    if up and st.button("RUN ANALYSIS"):
        ...
        # Prétraitement
        img_res = np.array(image.resize((224, 224))) / 255.0
        preds = model.predict(np.expand_dims(img_res, axis=0))[0]
        ...
        pdf_bytes = create_clinical_pdf(n, p, a, g, res_text, conf_val, image, date_now)
        st.download_button("📥 DOWNLOAD CLINICAL PDF", pdf_bytes, f"NeuroScan_{n}.pdf", "application/pdf")
    else:
        st.write("Ready for analysis.")

st.markdown(f'<p style="text-align:center; color:#4b5563; font-family:Times; margin-top:80px;">Designed by Douaa Houbad | M1 EMB Biomedical Engineer | {date_now}</p>', unsafe_allow_html=True)
